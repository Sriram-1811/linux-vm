...
check_ping() {
        ...
        ping_output=$($ping_cmd "$target" | tee -a "$log_file")
...
check_dns() {
        ...
        dns_output=$($dns_cmd "$target" | tee -a "$log_file")

        if ! echo "$dns_output" | grep -q "NXDOMAIN\|server can't find"; then
...
check_traceroute() {
        ...
        trace_output=$(timeout "$timeout" $traceroute_cmd "$target" 2>&1 | tee -a "$log_file")

        if echo "$trace_output" | awk 'NR>1 && $2 == "*" && $3 == "*" && $4 == "*" {next} {exit 1} END {exit 0}'; then
